...
class Network(nn.Module):
    ...
    def __init__(self, gpu_id ,stage=3):
        super(Network, self).__init__()
        self.stage = stage
        self.gpu_id = gpu_id
        self.enhance = EnhanceNetwork(layers=1, channels=3)
        self.calibrate = CalibrateNetwork(layers=3, channels=16)
        self._criterion = LossFunction(self.gpu_id)
...
class Finetunemodel(nn.Module):
    ...
    def __init__(self,gpu_id, weights):
        super(Finetunemodel, self).__init__()
        self.gpu_id = gpu_id
        self.enhance = EnhanceNetwork(layers=1, channels=3)
        self._criterion = LossFunction(self.gpu_id)

        base_weights = torch.load(weights) if int(self.gpu_id) != -1 else torch.load(weights, map_location = torch.device('cpu'))
