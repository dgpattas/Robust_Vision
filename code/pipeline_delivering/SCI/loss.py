...
class LossFunction(nn.Module):
    def __init__(self, gpu_id):
        ...
        self.smooth_loss = SmoothLoss(self.gpu_id)
...
class SmoothLoss(nn.Module):
    ...
    def rgb2yCbCr(self, input_im):
        ...
        mat = torch.Tensor([[0.257, -0.148, 0.439], [0.564, -0.291, -0.368], [0.098, 0.439, -0.071]]).cuda() if int(self.gpu_id) != -1 else torch.Tensor([[0.257, -0.148, 0.439], [0.564, -0.291, -0.368], [0.098, 0.439, -0.071]]).to(torch.device('cpu'))
        bias = torch.Tensor([16.0 / 255.0, 128.0 / 255.0, 128.0 / 255.0]).cuda() if int(self.gpu_id) != -1 else torch.Tensor([16.0 / 255.0, 128.0 / 255.0, 128.0 / 255.0]).to(torch.device('cpu'))
